...
def create_video(args: DeforumArgs, anim_args: DeforumAnimArgs):
    ...
    fps = anim_args.fps  # @param {type:"number"}
        ...
        if use_manual_settings:
            ...
            if render_steps:  # render steps from a single image
                ...
                mp4_path = os.path.join(
                    newest_dir, f"{args.timestring}_{path_name_modifier}.mp4"
                )
                max_frames = str(args.steps)
            else:  # render images for a video
                image_path = os.path.join(
                    args.outdir, f"{args.timestring}_%05d.{bitdepth_extension}"
                )
    ...
    #     process_gif = subprocess.Popen(
    #         cmd_gif, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    #     )
